...
void ReadConfig::ReadLine(string iline, ifstream *inFileStream)
{
  ...
  if( iline.find( "SINGLEPEAMPLSIGMA " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The sigma of the single PE pulse height distribution is "<< fSigmaSinglePEPulseHeightDistribution[i_telType]<<endl;
    }
  ...
  if( iline.find( "USEGAINDROP " ) < iline.size() )
    {
      i_stream >> i_char; i_stream >> i_char;
      i_stream >>  bUseGainDrop;
      cout<<"Do we want to use Gain drop: "<<bUseGainDrop<<endl;
    }
  ...
  if( iline.find( "DISCTHRESHOLD " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The Threshold of the discriminator in mV (ADC if 'camera snapshot' is active) is "<<fDiscThreshold[i_telType]<<endl;
    }
  ...
  if( iline.find( "DISCWIDTH " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The width of the discriminator output signal in ns is "<<fDiscWidth[i_telType]<<endl;
    }
  ...
  if( iline.find( "DISCDELAY " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The delay of the inverted signal in the CFD in ns is "<<fDiscDelay[i_telType]<<endl;
    }
  ...
  if( iline.find( "DISCATTENUATION " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The attenuation of the non-inverted signal in the CFD is "<<fDiscAttenuation[i_telType]<<endl;
    }
  ...
  if( iline.find( "DISCRFBDYNAMIC " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The initial dynamic value in the RFB in mV (will be mulitplied with 0.18 in the sims) is "<<fDiscRFBDynamic[i_telType]<<endl;
      cout<<"If RFB circuit is not used this stays constant"<<endl;
    }
  ...
  if( iline.find( "DISCPETOMVCONVERSION " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The conversion factor at the input of the discriminator mV per pe (Amplitude) is "<<fDiscPEtomVConversion[i_telType]<<endl;
    }
  ...
  if( iline.find( "FADCDCTOMVCONVERSION " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The conversion factor at the input of the FADC mV per dc (Amplitude) where the FADC response is linear is "<<fFADCdctomVConversion[i_telType]<<endl;
      cout<<"This number is used to pick the right pulse shape when assembling the trace. Note that for the trigger the high gain trace is used"<<endl;
    }
  ...
  if( iline.find( "SIMULATORNAME " ) < iline.size() )
    {
      ...
      cout<<"The Person executing the simulations is "<<sSimulatorName<<endl;
    }
  ...
  if( iline.find( "ATMOSPHERICMODEL " ) < iline.size() )
    {
      ...
      cout<<"The atmospheric model used is "<<iAtmosphericModel<<endl;
    }
  ...
  if( iline.find( "FADCLOHIGHGAINRATIO " ) < iline.size() )
    {
      ...
      cout<<"Telescope type "<<i_telType<<" The gain ratio between the low gain channel /  the high gain channel (amplitude) is "<<fFADCLowHiGainRatio[i_telType]<<endl;
    }
