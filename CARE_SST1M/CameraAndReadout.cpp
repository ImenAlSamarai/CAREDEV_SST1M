...
static const Bool_t DEBUG_FADC = kTRUE;
static const Bool_t DEBUG_TELTRIGGER = kTRUE;
static const Bool_t DEBUG_ARRAYTRIGGER = kTRUE;
static const Bool_t DEBUG_MAIN = kTRUE;
static const Bool_t DEBUG_TELESCOPEDATA = kTRUE;
  ...
  for(UInt_t t = 0 ; t<uNumTelescopeTypes; t++)
    {
      ...
      cout<<endl<<"Starting TraceGenerator for telescopetype "<<t<<endl;
  ...
  for(UInt_t t = 0 ; t<uNumTelescopeTypes; t++)
    {
      ...
      cout<<endl<<"Starting Trigger for telescope type "<<t<<endl;
      ...
      if(readConfig->GetVBFwriteBit())
	{
      ...
      for(UInt_t i = 0; i<uNumTelescopes; i++)  {
	    ...
	    if (bWriteTracesToRootFile)
	    {
		  ...
		  {
            FADC_name.Form("vFADCTraces%i",g);
            tout[i]->Branch(FADC_name,&(telData[i]->iFADCTraceInPixel[g]));
		  }
	    }
